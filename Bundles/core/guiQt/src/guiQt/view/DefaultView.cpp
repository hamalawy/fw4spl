...
void DefaultView::starting() throw(::fwTools::Failed)
{
    ...
    QWidget* container = this->getQtContainer();

    if(qApp->activeWindow()!=container)
    {
      std::cout<<" RECUPARATION CONTAINER  FAILED \n";
    }
    else
      std::cout<<"  CONTAINER  OK \n";

    // important le setParent
    m_manager = new QMainWindow();
    m_manager->setParent(container);
    
    
     QLayout *layout;
    
     if(container->layout()!=0)
        std::cout<<"DV  =====> Layout OK \n";
     else
       std::cout<<"DV  =====> Layout EMPTY \n";
     
     layout = container->layout();
     
    layout->setMenuBar(m_manager);
...
//	viewPanel->resize(250,250);
	    ...
	    viewPanel->setMinimumHeight(500);
	    viewPanel->setMinimumWidth(500);
//	    viewPanel->resize(500,500);
